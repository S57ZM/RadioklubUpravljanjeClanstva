...
| Nadgradnja varnostnih odvisnosti | ✅ | v1.28 | python-multipart 0.0.22→0.0.27 (DoS fix, Dependabot #8); pytest 8.3.4→9.0.3 (tmpdir fix, Dependabot #7, dev only) | **C1:** CSRF token dodan v obrazec za brisanje uporabnika (bug: brisanje ni delovalo). **H1:** Odjava GET→POST+CSRF (preprečitev prisilne odjave). **H2:** Rate limiting razširjen na `/profil/geslo`, `/profil/2fa-potrdi`, `/profil/2fa-onemogoči`. **H3:** Per-username lockout: `login_poskusi` dobi stolpec `uporabnisko_ime`; Alembic 009; nov modul `app/rate_limit.py`. 8 novih testov (169 skupaj) |
